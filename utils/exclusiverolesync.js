...
async function resolveCosmeticId(entry) {
  const directId =
    normalize(entry?.cosmetic?.id) ||
    normalize(entry?.cosmeticId);

  if (directId) return directId;

  const name =
    normalize(entry?.cosmetic?.name) ||
    normalize(entry?.cosmeticName);
...
function entryRoleLabel(entry) {
  ...
  if (entry?.roleLabel) return safeRoleName(entry.roleLabel);
...
function entryRareLabel(entry) {
  ...
  if (entry?.roleLabel) return String(entry.roleLabel);
...
async function snapshotOwnsEntry(snapshot, entry) {
  ...
  if (entry?.kind === "bundle") {
    ...
    return items.some((item) => {
      const id =
        normalize(item?.id) ||
        normalize(item?.cosmeticId) ||
        normalize(item);
      return id ? snapshotHasCosmeticId(snapshot, id) : false;
    });
  }
...
function getRoleEntries() {
  return (Array.isArray(EXCLUSIVES_LIST) ? EXCLUSIVES_LIST : []).map((entry) => ({
    ...
    rareLabel: entryRareLabel(entry),
  }));
}
